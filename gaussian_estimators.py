...
class UnivariateGaussian:
    ...
    def fit(self, X: np.ndarray) -> UnivariateGaussian:
        ...
        self.mu_ = X.mean()
        size_of_x = len(X)
        m = size_of_x if self.biased_ else (size_of_x - 1)
    ...
    def pdf(self, X: np.ndarray) -> np.ndarray:
        ...
        double_var = 2 * self.var_
        power1 = np.power((X - self.mu_), 2)
        return np.exp(- power1 / (double_var)) / np.sqrt(
            double_var * np.pi)
    ...
    def log_likelihood(mu: float, sigma: float, X: np.ndarray) -> float:
        ...
        """
        double_sigma = 2 * sigma
        power1 = np.power((X - mu), 2)
        first_exp = (len(X) / 2) * (-np.log(double_sigma * np.pi))
        second_exp = -np.sum(power1)
        return first_exp + second_exp / double_sigma
...
class MultivariateGaussian:
    ...
    def fit(self, X: np.ndarray) -> MultivariateGaussian:
        ...
        self.cov_ = (np.transpose((X - self.mu_)) @ (X - self.mu_)) / (
                len(X) - 1)
        self.fitted_ = True
        return self
    ...
    def pdf(self, X: np.ndarray):
        ...
        if not self.fitted_:
            raise ValueError(
                "Estimator must first be fitted before calling `pdf` function")
        matrix_mult = (X - self.mu_) @ (np.linalg.inv(self.cov_)) * (
                X - self.mu_)
        det_cov = np.linalg.det(
            self.cov_)
        sqrt1 = np.sqrt(np.power(2 * np.pi, len(X)) * det_cov)

        return np.exp(np.sum(-matrix_mult / 2, axis=1)) / sqrt1
    ...
    def log_likelihood(mu: np.ndarray, cov: np.ndarray,
        ...
        d = len(X[0])
        neg_m = -1 * len(X)
        log_expression = d * np.log(2 * np.pi) + np.log(np.linalg.det(cov))
        sum1 = np.sum((X - mu) @ np.linalg.inv(cov) * (X - mu))
        log_likelihood = (neg_m * log_expression - sum1) / 2
